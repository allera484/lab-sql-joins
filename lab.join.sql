...
GROUP BY COALESCE(category.name, 'No category');
...
SELECT 
category.name,
ROUND(AVG(length), 4)AS average_running_time
FROM film
...
-- and this information can be obtained using a CASE statement combined with IFNULL."
 ...
 SELECT 
  title,
  CASE WHEN inventory_id IS NULL THEN 'Not available' ELSE 'Available' END AS availability,
  CASE WHEN inventory_id IS NULL THEN 'Not available' ELSE 'Available' END AS availability_with_case,
  CASE WHEN inventory_id IS NOT NULL THEN inventory_id::text ELSE 'Not available' END AS availability_with_case_type_conversion
FROM inventory 
